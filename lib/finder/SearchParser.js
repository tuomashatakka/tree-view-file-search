...
let GitRepository, projectPath

try {
  GitRepository = require('atom').GitRepository
  projectPath = require('../utils')
}
catch(e) {
  console.warn("Error while requiring:", e)
}
...
class FileCache {
    ...
    constructor(options={}) {
      ...
      this.items = new List([])
      this.path = options.path || projectPath
    ...
    getIgnoredNames () {
      try {
        ...
        console.warn("getIgnoredNames -> atom.config.get failed:", e)
        return []
      }
    }
    ...
    isAllowed (path) {
      ...
      try {
        ...
        // atom.devMode &&
        console.error("Error while trying to resolve ignored paths:", e)
      }
...
export default function getCache(path='.') {
  return _cache
}
